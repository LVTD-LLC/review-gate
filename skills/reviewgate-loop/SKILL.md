...
Review Gate's loop contract is:

1. Read the latest Review Gate JSON artifact at `.reviewgate/review.json` when available.
2. If the JSON artifact is unavailable, read the canonical PR summary comment containing `<!-- review-gate-summary -->`.
...
6. Commit and push.
7. Wait for Review Gate to update the same summary comment.
