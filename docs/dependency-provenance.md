...
The lockfile was regenerated with Cargo 1.96 from the public crates.io
registry using:
...
The current Serde dependency shape is the dependency graph Cargo resolves for
the locked versions:
...
Re-run those checks with `--locked` when auditing the lockfile:

```bash
cargo metadata --no-deps --format-version 1 --locked
cargo tree -i serde_core --locked
cargo tree -i zmij --locked
```
